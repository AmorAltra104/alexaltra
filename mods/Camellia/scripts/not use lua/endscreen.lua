...
local Panel_Color = 'ffffff'
local FontSize=65
local MiscFontSize = 65
local ScoreFontSize = 100
local ScoreFont ='1.ttf'
...
function onCreate()
	...
	makeLuaSprite('GradientFadeIn', 'end/EndScreenGradient', 0, 0)
	makeLuaSprite('ColorBgSlider', 'end/EndScreenBG', 3280, 0)
	makeLuaSprite('PanelSlider', 'end/EndScreenPanel', -2000, 0)
	makeLuaSprite('Vicotory', 'end/EndScreen_Vicotory', 0, -720)
	
	makeLuaSprite('RatingsSick', 'end/EndRatings_Sick', -1000, 144*0)
	makeLuaSprite('RatingsGood', 'end/EndRatings_Good', -1000, 144*1)
	makeLuaSprite('RatingsBad', 'end/EndRatings_Bad', -1000, 144*2)
	makeLuaSprite('RatingsShit', 'end/EndRatings_Shit', -1000, 144*3)
	makeLuaSprite('RatingsMiss', 'end/EndRatings_Miss', -1000, 144*4)
	...
	setTextFont('EndScreen_Score', '1.ttf')
	setTextFont('EndScreen_Sick', '1.ttf')
	setTextFont('EndScreen_Good', '1.ttf')
	setTextFont('EndScreen_Bad', '1.ttf')
	setTextFont('EndScreen_Shit', '1.ttf')
	setTextFont('EndScreen_Miss', '1.ttf')
...
function onStepHit()
	if getPropertyFromClass('flixel.FlxG', 'sound.music.time') >= songLength-100  and not isStoryMode then --checks if theres 1 second left in the song
		triggerEvent('PlayAnimation', 'RatingScreen','')
...
function onEvent(name, value1, value2)
	...
	if name == 'PlayAnimation' and value1 == 'RatingScreen' then
	
		StoreTime()
		FreezeTime=true
		playSound('breakfast', 0.3, true)
		...
		GrabRatings()
		makeLuaSprite('RankThingy', 'end/Ranking_'..Rank, 2000, 350)
...
function onUpdate()
	...
	if getPropertyFromClass('flixel.FlxG', 'keys.justPressed.SPACE') or (getMouseX('camHUD') > 0 and getMouseX('camHUD') < 1280) and (getMouseY('camHUD') > 0 and getMouseY('camHUD') < 310 and mouseClicked('left')) and FreezeTime==true then
		FreezeTime=false
		SeenRatingScreen=true
